...
die_t die_create(distribution_t* distr) {
    ...
    for (size_t i = 0; i < distr->weights.size; i++)
        weightsum += *(size_t*)array_get(&distr->weights, i);
    for (size_t i = 0; i < distr->weights.size; i++)
        array_add(&die.sides, &(double){ *(size_t*)array_get(&distr->weights, i) / (double)weightsum });
    return die;
}
...
size_t dice(die_t* die) {
    ...
    for (; side < die->sides.size; side++) {
        offset += *(double*)array_get(&die->sides, side);
        if (random < offset)
            return side;
    }
    return side;
