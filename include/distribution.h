...
typedef enum distr_preset_t {
    ...
    DISTR_PRESET_UNIFORM,               // The uniform preset describes a distribution with only equal weights.
    DISTR_PRESET_TWODICE                // The twodice preset describes a distribution that arises when using two uniform and equal dice of equal side count and using the sum of the two diced values as result.
} distr_preset_t;
...
/**
 * Creates an empty distribution with preset DISTR_PRESET_NONE and an empty array of weights with elementsize = sizeof(size_t).
 * @return The created distribution.
 */
distribution_t distr_create_empty();

/**
 * Creates a distribution with the given preset and an empty array of weights with elementsize = sizeof(size_t).
 ...
 * If the distribution has preset
 * 
 ...
 * 
 * DISTR_PRESET_UNIFORM die sides many weights of value 1 are added creating a uniform distribution. 
 * 
